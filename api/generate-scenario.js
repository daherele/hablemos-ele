...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2) Reintento con temperatura baja
    if (!parsed.ok) {
    ...
    scenario.title =
      String(scenario.title || "").trim() || `Situación: ${context}`;

    scenario.description =
      String(scenario.description || "").trim() || "Escenario generado con IA.";
    ...
    scenario.objectives = Array.isArray(scenario.objectives)
      ? scenario.objectives
      : [];
